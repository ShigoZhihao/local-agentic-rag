...
> **謝辞**
>
> このプロジェクトは [DeepLearning.AI](https://www.deeplearning.ai/) が公開している
> **[Retrieval Augmented Generation (RAG) Course](https://www.deeplearning.ai/courses/retrieval-augmented-generation-rag/)**
> の内容をベースに構築しています。
>
> これほど充実したコースを無償で公開してくださった DeepLearning.AI チームに心より感謝いたします。
> 丁寧かつ実践的なカリキュラムのおかげで、本格的な RAG システムをゼロから構築することができました。
...
> **現在のモデル選定はテスト用途。** 実用時は planner を `qwen2.5:27b`、executor を `qwen2.5:9b` に変更推奨。
...
| Embedding | BAAI/bge-m3 (1024 次元, sentence-transformers, GPU) |
...
| `retrieval.hybrid.alpha` | BM25/Semantic バランス (0=BM25, 1=Semantic) | 0.5 |
| `retrieval.bm25.k1` | BM25 単語頻度飽和パラメータ | 1.2 |
| `retrieval.bm25.b` | BM25 文書長正規化パラメータ | 0.75 |
| `reranking.top_k` | Re-rank 後の返却件数 | 5 |
| `agents.max_loop_count` | Validator→Facilitator 最大ループ回数 | 3 |
| `agents.validation_threshold` | Validator 合格ライン (平均スコア) | 80 |
