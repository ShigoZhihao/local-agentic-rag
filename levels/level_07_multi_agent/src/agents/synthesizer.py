...
def _assess_and_route(
    ...
    response = call_executor(messages, json_mode=True)
    raw = response.answer
    ...
    if response.thinking:
        agent_thinking.append({"agent": "Synthesizer", "thinking": response.thinking})
    ...
    if can_answer and confidence >= 70:
        ...
        gen_response = call_executor(answer_messages)
        answer = gen_response.answer
        if gen_response.thinking:
            agent_thinking.append({"agent": "Synthesizer", "thinking": gen_response.thinking})
...
def _generate_with_citations(
    ...
    gen_response = call_executor(messages)
    answer = gen_response.answer
