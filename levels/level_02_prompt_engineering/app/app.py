"""Level 2 — Prompt Engineering: Reflex chat UI entry point.
...
def sidebar() -> rx.Component:
    """Render settings sidebar with model, prompt mode, and parameters."""
    return rx.box(
        rx.vstack(
            ...
            rx.vstack(
                rx.text("Prompt Mode", weight="bold", size="2"),
                rx.select(
                    ChatState.prompt_modes,
                    value=ChatState.prompt_mode,
                    on_change=ChatState.set_prompt_mode,
                    width="100%",
                ),
                rx.text(
                    "basic / cot / few_shot / structured",
                    size="1", color="gray",
                ),
                spacing="1",
            ),
...
app.add_page(
    index,
    title="Level 2 — Prompt Engineering",
